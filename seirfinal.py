...
def seir_model_births_deaths():
    ...
    for step in range(num_steps):
        tot_pop = s[step] + e[step] + i[step] + r[step]
        ...
        s2e = h * contacts_per_day * transmission_probability * i[step] * (s[step]/tot_pop)
        e2i = h / latency_time * e[step]
        i2r = h / infectious_time * i[step]
        # Task 2: Include births and deaths in the following four lines.
        # Modify the code below:
        s[step + 1] = s[step] - s2e + h*birth_rate*(tot_pop) - h*mortality_rate*s[step]
