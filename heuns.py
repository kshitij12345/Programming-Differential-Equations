...
def heuns_method(num_steps):
    ...
    xH = numpy.zeros([num_steps + 1, 2]) # m
    vH = numpy.zeros([num_steps + 1, 2]) # m / s
    ...
    ###Heun's Method
    for step in range(num_steps):
        initial_acc = acceleration(x[step])
        xE = x[step] + h*v[step]
        vE = v[step] + h*initial_acc
        #print("xE,vE",xE,vE)
        x[step + 1] = x[step] + h * (v[step]+vE)*0.5
        v[step + 1] = v[step] + h * (initial_acc + acceleration(xE))*0.5
